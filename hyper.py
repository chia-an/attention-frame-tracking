...
def objective(configs):
    ...
    Train the model with different seeds then average the score.
    """
    ...
    results = {
        ...
        'general': 82.4,
        'query-key': 82.7,
        'scaled-query-key': 82.5,
    }

    if configs['input_dim'] == 128 and \
       configs['hidden_dim'] == 128 and \
       configs['embed_dim'] == 128 and \
       configs['att_dim'] == 64:
        return {'loss': -results[configs['attention_type']],
                'status': STATUS_OK,
                'input': configs,}
    ...
    for run in range(n_runs):
        ...
        kwargs = set_attention(configs['attention_type'])(kwargs)
        kwargs = set_input_dim(configs['input_dim'])(kwargs)
        kwargs = set_hidden_dim(configs['hidden_dim'])(kwargs)
        kwargs = set_embed_dim(configs['embed_dim'])(kwargs)
        kwargs = set_att_dim(configs['att_dim'])(kwargs)
        ...
        vs = history['history'][('frames-user-slot', 'accuracy')]
        best_epoch, _ = max(enumerate(vs), key=lambda x: x[1])

        vs = history['history'][('test-frames-user-slot', 'accuracy')]
        accuracies.append(vs[best_epoch])

    import numpy as np
    return {'loss': -np.array(accuracies).mean(),
            'status': STATUS_OK,
            'input': configs,
            'raw_accs': list(accuracies)}


def main():
    dims = [64, 128, 256, 512]
    space = hp.choice('is_qk', [
        {
            'attention_type': hp.choice('att_type',
                ['no', 'simple', 'general', 'scaled-simple', 'content', 'cosine']),
            'input_dim': hp.choice('input_dim', dims),
            'hidden_dim': hp.choice('hidden_dim', dims),
            'embed_dim': hp.choice('embed_dim', dims),
            'att_dim': 64,
        },
        {
            'attention_type': hp.choice('qk_att_type',
                ['query-key', 'scaled-query-key']),
            'input_dim': hp.choice('qk_input_dim', dims),
            'hidden_dim': hp.choice('qk_hidden_dim', dims),
            'embed_dim': hp.choice('qk_embed_dim', dims),
            'att_dim': hp.choice('qk_att_dim', [32, 64]),
        },
    ])
